...
if (isset($_POST['update'])){
    ...
    if ($initpassword=='') {
        ...
        conf($self.'?_clid='.$cmd,'e',"Both the Passwords Do not Match");
    }
    else{
...
//check if a client already exist with the same email id
    if ($email!='' AND $email!=$cemail){
        ...
        if ($cnt!='0'){
            conf($self.'?_clid='.$cmd,'e',"This Email id is already registered with a Client");
        }
    }
    ...
    $aclient->save();
    $did = $aclient->id();
    conf("client-manage.php?_clid=$did",'s','Client Updated Successfully');
}
else{
