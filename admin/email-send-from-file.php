...
if (isset($_POST['submit'])){
    ...
    if($subject==''){
        conf($self,'e','Please Insert the Email Subject');
    }
    if($message==''){
        conf($self,'e','Please Insert the Email Message');
    }
    ...
    if((!empty($_FILES["uploaded_file"])) && ($_FILES['uploaded_file']['error'] == 0)) {
        ...
        else { exit ("Invalid file type. Please try again");}
    ...
    for($i = 0; $i < $listscount; $i++){
        ...
        if ($listscount> 0) {
            ...
            conf('email-history.php','s','Email Sent Successfully');
            }

        }
        else{
            conf($self,'e','There is No Email in this File');
        }
        }
        else{
